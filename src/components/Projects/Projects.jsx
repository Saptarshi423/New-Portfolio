import React, { useState } from "react";
import "./Projects.css";

export const projectsData = [
  {
    projectName: "Compfy SLoth",
    projectDescription:
      "An ecommerce web application built using React, Redux. Implemented authorization using Auth0 and ability to filter, add, to cart, remove from cart etc.",
    imageUrl:
      "https://github.com/user-attachments/assets/08a87439-bc7f-40d1-b547-6b45f0d4682d",
    projectUrl: "https://github.com/Saptarshi423/Comfy-sloth/tree/main",
  },
  {
    projectName: "Multiplayer Tic Tac Toe",
    projectDescription:
      "A multiplayer game enabling 2 player to play together. Tech Stach:- React, TypeScript, CSS, SocketIO, NodeJS",
    imageUrl:
      "https://github.com/user-attachments/assets/32ef7cdb-9766-498b-a621-9b1a5c87c4c5",
    projectUrl: "https://github.com/Saptarshi423/TicTacToe-Client",
  },
  {
    projectName: "ExpenseTracker",
    projectDescription:
      "I present you the Expense Tracker app, where you can keep track of your Income and expenses",
    imageUrl:
      "https://user-images.githubusercontent.com/51373298/160224929-592e66b4-f8e7-4d2d-9a88-d85f1ddaedc1.PNG",
    projectUrl: "https://github.com/Saptarshi423/ExpenseCalculator",
  },
  {
    projectName: "Metro ShopApp",
    projectDescription:
      "Web based ecommerce application. The app come with features that allow publishing of new product entries, adding product pictures. Products are managed through the admin.",
    imageUrl:
      "https://user-images.githubusercontent.com/51373298/151177220-08cbd0f3-f921-418d-a52f-bd192bfcc508.JPG",
    projectUrl: "https://github.com/Saptarshi423/MetroShopp",
  },
  {
    projectName: "Color Generator",
    projectDescription:
      "As a developer do you find it difficult to find the correct hexcode/color for your website. So here is a app that can help you generate all the relative color for any particular hex value. Super Cool !!",
    imageUrl:
      "https://user-images.githubusercontent.com/51373298/160225137-c767163a-53d5-413e-be86-318cb112f0f3.PNG",
    projectUrl: "http://theleanprogrammer.com/flex/",
  },
  {
    projectName: "Telegram Bot",
    projectDescription:
      "A Telegram bot which can be used to find ;the meaning of different Dictionary Words!",
    imageUrl: "https://miro.medium.com/max/1400/1*UKPKu1bycj79C-tNx96-Vg.png",
    projectUrl: "https://github.com/Saptarshi423/Bot",
  },
  {
    projectName: "Timer Watch",
    projectDescription:
      "A simple timer app to keep track of you time. Tech Stack:- React, TypeScript, CSS, HTML",
    imageUrl:
      "https://github.com/user-attachments/assets/7866d086-0387-482d-9678-27d607fff4a5",
    projectUrl: "https://github.com/Saptarshi423/stop-watch",
  },
];

const CARDS_PER_PAGE = 3;

const Projects = () => {
  const [page, setPage] = useState(0);
  const totalPages = Math.ceil(projectsData.length / CARDS_PER_PAGE);

  const handleDotClick = (idx) => setPage(idx);

  // For sliding effect
  const slideWidth = 340; // card width + gap (adjust if needed)
  const translateX = -(page * slideWidth * CARDS_PER_PAGE);

  return (
    <section id="projects" className="projects-section">
      <h2 className="projects-title">Projects</h2>
      <div className="carousel-viewport">
        <div
          className="carousel-track"
          style={{
            transform: `translateX(-${page * 100}%)`,
          }}
        >
          {projectsData.map((project, idx) => (
            <div className="project-card" key={project.projectName}>
              <a href={project.projectUrl} target="_blank" rel="noopener noreferrer">
                <img loading="lazy" src={project.imageUrl} alt={project.projectName} className="project-img" />
              </a>
              <div className="project-info">
                <h3>{project.projectName}</h3>
                <p>{project.projectDescription}</p>
              </div>
            </div>
          ))}
        </div>
      </div>
      <div className="carousel-dots">
        {Array.from({ length: totalPages }).map((_, idx) => (
          <button
            key={idx}
            className={`dot${idx === page ? " active" : ""}`}
            onClick={() => handleDotClick(idx)}
            aria-label={`Go to page ${idx + 1}`}
          />
        ))}
      </div>
    </section>
  );
};

export default Projects;